...
class SequenceTagger(flair.nn.Classifier[Sentence]):
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                if return_loss:
                    ...
                    overall_loss += loss[0]
                    label_count += loss[1]
                batch_labels = torch.ceil(gold_labels/4).detach().cpu().numpy().tolist()
                ...
                if self.use_crf:
                    ...
                    predictions, all_tags, batch_prob = self._standard_inference(
            ...
            if return_loss:
                return (overall_loss, label_count), batch_labels, batch_prob
            
    # Changes
    # Group BIES probabilities into original class probabilities
    def Group_BIES_Probabs(self, softmax_batch):
        batch_prob = []
        for token in softmax_batch:
            token = token.detach().cpu().numpy().tolist()
            token_prob = []
            token_prob.append(token[0])
            for i in range(1, len(token), 4):
                token_prob.append(sum(token[i:i+4]))
            batch_prob.append(token_prob)
        return batch_prob
    ...
    def _standard_inference(self, features: torch.Tensor, batch: List[Sentence], probabilities_for_all_classes: bool):
        ...
        # Changes
        batch_prob = self.Group_BIES_Probabs(softmax_batch)      
        ...
        return predictions, all_tags, batch_prob
